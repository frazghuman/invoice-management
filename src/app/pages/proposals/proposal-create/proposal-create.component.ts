...
export class ProposalCreateComponent {
  ...
  submitProposal() {
    ...
    if (this.proposalForm.valid && this.proposalForm.value.items.length) {
        console.log(this.proposalForm.value);
        this.createProposal(this.proposalForm.value);
    } else {
  ...
  createProposal(data: any) {
    this.proposalsService.createProposal$(data).subscribe({
      next: (response) => {
        if (response?._id) {
          const { _id } = response;
